...
X.unit <- matrix(0, 1, X.dim) # X unit
y.unit <- matrix(0, 1, 1)     # y unit
X.signal <- matrix(0, train.size-1, X.dim) # X signal
y.signal <- matrix(0, train.size-1, 1)     # y signal
X.norm <- matrix(0, train.size-1, X.dim)   # X normalized
y.norm <- matrix(0, train.size-1, 1)       # y normalized
...
y.train.hat <- matrix(0,train.size-1,1) # prediction of y.train 
